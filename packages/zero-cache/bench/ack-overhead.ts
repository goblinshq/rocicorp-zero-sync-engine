...
type Strategy = 'per-message-ack' | 'production-cumulative-ack';
...
const strategyOrder: readonly Strategy[] = [
  'per-message-ack',
  'production-cumulative-ack',
];
...
function makeDecision(
  ...
  return {
    go,
    recommendation: go
      ? 'GO for production cumulative ack; production stream helpers reduce ack overhead without material latency regression.'
      : 'NO-GO for production cumulative ack; revisit batching defaults or keep per-message ack semantics.',
    threshold,
  };
}
...
export async function main() {
  ...
  for (const scenario of scenarios) {
    for (const payload of payloads) {
      const [baseline, cumulative] = await runScenarioPair(
        scenario,
        payload,
        messages,
      );
